...
int millis_stop = 0;
unsigned long long int millis_10_stop = 0;
unsigned long long int second_stop = 0;
unsigned long long int minute_stop = 0;

int first_read_change = 0;
int second_read_change = 0;
int first_read_sel = 0;
int second_read_sel = 0;
int init_setting = 0;
...
int hour_flag = 0;
int button_flag = 3;
int stop_watch_flag = 0;
int second_stop_flag = 0;
int minute_stop_flag = 0;
int change_state = 0;
# 10 "blink_manager.c" 2
...
# 12 "blink_manager.c" 2

# 1 "./time_manager.h" 1
# 13 "blink_manager.c" 2
...
void display_clock(int type_display, int state_display);
void change_time(void);
void blink(int state, int type);
void blinky(void);
void init_stop_watch();
# 14 "blink_manager.c" 2


void blink(int state, int type){
    if (change_state == 1){
        display_clock(5 - state_machine, 1);
        LATD++;
        change_state = 0;
        return;
    }
...
void blinky(void){
    blink_count++;
    if (blink_count >= 25 && blink_state == 1){
    ...
    if (blink_count >= 25 && blink_state == 0){
        switch(state_machine){
            case MODE_1:
                blink(1,1);
                break;
