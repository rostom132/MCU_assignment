...
uint32_t register_timer(uint64_t delay,uint64_t period,timer_callback_t callback, void *data ){
    if (NUMBER_EXIST_TASKS >= 10 ) return 0;
    NUMBER_EXIST_TASKS ++;
    int i =0;
    while(1){
        if (tasks[i].period == 0) {
...
int remove_timer (uint32_t id){
    ...
    while(1){
        if (tasks[temp_index].next == id){
            tasks[id].callback = ((void*)0);
            tasks[id].period = 0;
            tasks[tasks[id].next].delay += tasks[id].delay;
            tasks[id].delay = 0;
            tasks[id].data = ((void*)0);
...
int timer_ISR(){
    tasks[HEAD_QUEUE].delay = tasks[HEAD_QUEUE].delay - 10;
    int i = 0;
